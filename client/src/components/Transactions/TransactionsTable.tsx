...
const TransactionsTable: FC<TransactionsTableProps> = ({ transactions }) => {
  const TableHeadCell: FC<{ children: React.ReactNode }> = ({ children }) => (
    <th className="text-center">{children}</th>
  );
  const TableCell: FC<{ className?: string; children: React.ReactNode }> = ({
    children,
    className,
  }) => (
          ...
          <tbody>
            {transactions.map((transaction) => (
              <tr key={transaction.id}>
                ...
                <TableCell>
                  {Format.trim(transaction.description, 50)}
                </TableCell>
                <TableCell
                  className={`${
